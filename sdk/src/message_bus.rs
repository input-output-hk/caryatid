...
pub type ObserverFn<M> = dyn Fn(Arc<M>) -> BoxFuture<'static, ()> + Send + Sync + 'static;
...
pub trait MessageBus<M: MessageBounds>: Send + Sync {
    ...
    fn register_observer(&self, topic: &str, observer: BoxedObserverFn<M>) -> Result<()>;
}

// Extension trait to sugar registration - needed because MessageBus must be object-safe
// to be used dynamically, which means we can't accept closures through type parameters
...
impl<M: MessageBounds> MessageBusExt<M> for Arc<dyn MessageBus<M>> {
    ...
    where
        F: Fn(Arc<M>) + MessageBounds,
    {
        let boxed_observer: BoxedObserverFn<M> = Box::new(move |message: Arc<M>| {
