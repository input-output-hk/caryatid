            ...
            match read_to_string(&filename) {
                Ok(file) => {
                    match serde_json::from_str::<Arc<M>>(&file) {
                        Ok(message) => {
                            match context.message_bus.publish(&topic, message).await {
                                Err(error) => {
                        ...
                        Err(error) => {
                            error!("Failed to playback message to file {:?}: {}. Aborting playback", &filename, error);
                            break;
                        }
                    };
                },
